...
class ProductsSheetImport implements ToModel, WithHeadingRow
{
    ...
    private function createProduct(array $row, $productName, $excelProductId)
    {
        ...
        // ✅ नया: is_sale और is_gift fields
        $isSale = $this->parseBoolean($this->getValue($row, ['Is Sale', 'is_sale'], 'No'));
        $isGift = $this->parseBoolean($this->getValue($row, ['Is Gift', 'is_gift'], 'No'));
        ...
        foreach ($optionalFields as $dbField => $excelFields) {
            ...
            elseif (in_array($dbField, $floatFields)) {
                ...
                // ✅ Special case: shop_zone_id validate with FK
                if ($dbField === 'shop_zone_id') {
                    $zoneId = is_numeric($value) ? (int)$value : null;

                    if ($zoneId && \App\Models\ShopZone::where('zone_id', $zoneId)->exists()) {
                        $productData[$dbField] = $zoneId;
                    } else {
                        // Agar invalid hai to skip / null
                        $productData[$dbField] = null;

                        Log::warning("Invalid shop_zone_id skipped: {$value} (Excel Product ID: {$excelProductId})");
                    }
    ...
    private function updateProduct(Product $product, array $row)
    {
        ...
        foreach ($updatableFields as $field => $keys) {
            ...
            if ($value !== null && $value !== '' && $value !== 'N/A') {
                ...
                } elseif (in_array($field, ['is_sale', 'is_gift', 'is_featured', 'ready_to_ship', 'is_collection'])) {
                    $update[$field] = $this->parseBoolean($value);
                } elseif (in_array($field, ['delivery_days', 'sort_order', 'shop_zone_id'])) {
...
class VariationsSheetImport implements ToModel, WithHeadingRow
{
    ...
    public function __construct(CombinedProductsImport $combinedImport)
    {
        ...
        $this->taxRates = TaxRate::pluck('id', 'code')->toArray(); // ✅ Tax rates by code
    }
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            // ✅ नया: Making charges
            $makingCharges = (float)$this->getValue($row, ['Making Charges', 'making_charges'], 0);

            // ✅ नया: Tax rate codes (comma separated)
            $taxRateCodes = $this->getValue($row, ['Tax Rate Codes', 'tax_rate_codes']);
            ...
            $variationData = [
                ...
                'making_charges' => $makingCharges, // ✅ नया field
                'sku' => $sku,
            ...
            // ✅ Tax rates attachment
            if (!empty($taxRateCodes) && $taxRateCodes !== 'N/A') {
                $codes = array_map('trim', explode(',', $taxRateCodes));
                $taxRateIds = [];

                foreach ($codes as $code) {
                    if (isset($this->taxRates[$code])) {
                        $taxRateIds[] = $this->taxRates[$code];
                    } else {
                        Log::warning("Tax rate code '{$code}' not found for variation SKU: {$sku}");
                    }
                }

                if (!empty($taxRateIds)) {
                    $variation->taxRates()->sync($taxRateIds);
                    Log::info("Attached tax rates to variation: " . implode(',', $taxRateIds));
                }
            }
