...
class VariationsSheetImport implements ToModel, WithHeadingRow
{
    ...
    private $metalColors;
    private $diamondQualities;
    private $taxRates; // ✅ Add tax rates cache
    private $rowNumber = 0;
    ...
    public function __construct(CombinedProductsImport $combinedImport)
    {
        ...
        $this->metalColors = MetalType::pluck('dmt_id', 'dmt_name')->toArray();
        $this->diamondQualities = DiamondQualityGroup::pluck('dqg_id', 'dqg_name')->toArray();
        $this->taxRates = TaxRate::pluck('id', 'code')->toArray(); // ✅ Load tax rates by code
    }
    ...
    public function model(array $row)
    {
        ...
        try {
            ...
            // ✅ Get tax rate codes from Excel
            $taxRateCodes = $this->getValue($row, ['Tax Rate Codes', 'tax_rate_codes', 'Tax Codes', 'tax_codes']);
            $taxRateIds = [];

            if ($taxRateCodes && $taxRateCodes !== 'N/A') {
                // Split comma-separated tax codes
                $codes = explode(',', $taxRateCodes);
                foreach ($codes as $code) {
                    $code = trim($code);
                    if (isset($this->taxRates[$code])) {
                        $taxRateIds[] = $this->taxRates[$code];
                    } else {
                        // Try to find by ID if code not found
                        $taxRate = TaxRate::where('code', $code)->orWhere('id', $code)->first();
                        if ($taxRate) {
                            $taxRateIds[] = $taxRate->id;
                            $this->taxRates[$taxRate->code] = $taxRate->id;
                        }
                    }
                }
            }
            ...
            if ($existingVariation) {
                ...
                // ✅ Update tax rates
                if (!empty($taxRateIds)) {
                    $existingVariation->taxRates()->sync($taxRateIds);
                }
            } else {
                // Create new variation
                $variationData['created_at'] = now();
                ...
                // ✅ Attach tax rates
                if (!empty($taxRateIds)) {
                    $variation->taxRates()->attach($taxRateIds);
                }
            }
